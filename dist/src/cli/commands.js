...
function renderProgressBar(score, width = 25) {
    ...
    program
        ...
        try {
            ...
            if (result.files.untracked.length) {
                ...
                console.log(chalk.dim("  (Use 'git add <file>...' to include in what will be committed)"));
            ...
            console.log(chalk.dim("─".repeat(40)));
            console.log(`${chalk.white.bold(" Commit Readiness:")}  ${scoreColor(`${readiness.score}%\n`)}`);
            ...
            if (readiness.warnings.length) {
                ...
                console.log(`  ${chalk.green.bold("✔ Complete green flag! Ready to confidently commit.")}\n`);
            }
        }
        catch (error) {
...
Run ${chalk.bold.white("git init")} inside your project root to initialize one.
    ...
    `));
            ...
            if (analysis.impact === "MEDIUM")
                impactColor = chalk.yellow;
            if (analysis.impact === "HIGH")
                impactColor = chalk.red.bold;
